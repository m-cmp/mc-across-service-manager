...
import { useGetMemUsedByInsName } from '@/network/apis/useMonitoringAcrossService';

interface Props {
  instanceName: string;
}

export default function MemAvgGauge({ instanceName }: Props) {
  const result = useGetMemUsedByInsName(instanceName);
  ...
  return (
    <MainCard title="Memory 사용률(%) - 1m" content={true}>
