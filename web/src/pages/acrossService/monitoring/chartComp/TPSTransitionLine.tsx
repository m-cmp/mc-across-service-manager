import { useGetTpsTranstionByIns } from '@apis/useMonitoringAcrossService';
...
export default function TPSTransitionChart({ instanceName }: Props) {
  const result = useGetTpsTranstionByIns(instanceName);
    ...
    function getSeries(): SeriesOption[] | undefined {
      if (result.isSuccess) {
        ...
        const datas = result.data.reduce<{ [key: string]: number[] }>((acc, item) => {
          ...
          if (typeof item._value === 'number' && item._value > 0) {
            acc[field].push(item._value / 10);
          }
          return acc;
        }, {});
  ...
  if (series.length === 0) {
    return (
      <MainCard title="TPS 추이 - 5m" content={true}>
        <div>데이터가 없습니다.</div>
      </MainCard>
    );
  }
  return (
    <MainCard title="TPS 추이 - 5m" content={true}>
      <LineChart loading={loading} xAxis={x} series={series} isLegendShow={false} />
    </MainCard>
  );
}
