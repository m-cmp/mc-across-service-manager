import { useGetActiveTransByIns } from '@apis/useMonitoringAcrossService';
...
export default function ActiveTransitionChart({ instanceName }: Props) {
  const result = useGetActiveTransByIns(instanceName);
  ...
  if (series.length === 0) {
    return (
      <MainCard title="Active 추이 - 5m" content={true}>
        <div>데이터가 없습니다.</div>
      </MainCard>
    );
  }
