...
with DAG(
    dag_id = 'destroy_across_api_call_template_dag', 
    tags = ["api_template"], 
    ...
    with TaskGroup(group_id='terraform_task_group') as terraform:
        ...
        SimpleHttpOperator.partial(
            ...
            log_response = True,
            method = "DELETE",
            endpoint = "/api/v1/multi-cloud/across-services?vmOnlyYn={{dag_run.conf['vm_only_yn'] if dag_run.conf['vm_only_yn'] else 'N'}}",
    ...
    def get_json_list(**context):
        ...
        for service in service_list:
            if service_list.index(service) == len(service_list) - 1:
                data += service.get('across_service_id')
            else:
                data += service.get('across_service_id') + ","
    ...
    delete_service_instance = SimpleHttpOperator(
        ...
        method = "DELETE",
        retries=0,
        endpoint = "/api/v1/orchestrator/info/across-services?across_service_id={{ti.xcom_pull(task_ids='get_json_list')}}"
    )
